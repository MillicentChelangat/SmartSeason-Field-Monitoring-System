...
import {
  ...
  BarChart2, FileText, LogOut, Menu,
} from 'lucide-react';
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', background: '#eef2ee', fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <aside style={{
        ...
        {/* Nav */}
        <nav style={{ flex: 1, overflowY: 'auto', padding: '8px 12px', whiteSpace: 'nowrap' }}>
            ...
            <div key={section} style={{ marginBottom: 4 }}>
              ...
              {NAV_ITEMS.filter(i => i.section === section).map(({ id, label, icon: Icon }) => {
                ...
                return (
                  ...
                  >
                    <Icon size={16} strokeWidth={isActive ? 2 : 1.6} style={{ flexShrink: 0 }} />
        ...
        {/* User + Logout */}
        <div style={{ padding: '10px 12px 14px', borderTop: '1px solid #f0f2ee', flexShrink: 0, whiteSpace: 'nowrap' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '8px 10px', borderRadius: 9, background: '#f9fafb', marginBottom: 4 }}>
            <div style={{
              ...
              fontSize: 11, color: '#a8e6be', fontWeight: 700,
            }}>
              {user?.full_name ? initials(user.full_name) : 'A'}
            </div>
            <div style={{ minWidth: 0 }}>
              <p style={{ fontSize: 12.5, color: '#111', fontWeight: 600, margin: 0, overflow: 'hidden', textOverflow: 'ellipsis' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, height: '100vh', overflowY: 'auto', position: 'relative' }}>
        ...
        {/* Hamburger — floats over the page topbar on the left */}
        <button
          onClick={() => setSidebarOpen(prev => !prev)}
          style={{
            position: 'absolute', top: 13, left: 14, zIndex: 20,
        ...
        {/* Page content — each page renders its own topbar + body */}
        <div style={{ flex: 1, padding: '12px 16px 24px 56px' }}>
          {children}
        </div>
      </div>
    </div>
