      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 20, padding: '24px 24px 40px' }}>
        ...
        <div style={{ background: 'linear-gradient(135deg,#0f2e1a,#1d6b35)', borderRadius: 14, padding: isMobile ? '18px 18px' : '22px 28px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', overflow: 'hidden', position: 'relative' }}>
          ...
          <div>
            ...
            <p style={{ fontFamily: "'Syne', sans-serif", fontSize: isMobile ? 18 : 22, fontWeight: 700, color: '#fff', lineHeight: 1.2 }}>
              {user?.full_name?.split(' ')[0] || 'Admin'}
            </p>
            <p style={{ fontSize: isMobile ? 11 : 12, color: 'rgba(255,255,255,0.5)', marginTop: 6 }}>
              You have {fields.length} fields and {agentCount} agents active
            </p>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : 'repeat(4, 1fr)', gap: isMobile ? 10 : 16 }}>
          {STAT_CARDS.map(({ label, value, icon, trend, trendUp }, i) => {
            ...
            return (
              <div key={label} style={{ background: s.bg, borderRadius: 14, padding: isMobile ? '14px' : '20px', boxShadow: '0 4px 16px rgba(0,0,0,0.12)', position: 'relative', overflow: 'hidden' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                  <span style={{ fontSize: isMobile ? 9.5 : 11, fontWeight: 600, color: s.subColor, textTransform: 'uppercase', letterSpacing: 0.5 }}>{label}</span>
                  <div style={{ width: 30, height: 30, borderRadius: 8, background: s.iconBg, display: 'flex', alignItems: 'center', justifyContent: 'center', color: s.iconColor }}>
                    {icon}
                  </div>
                </div>
                <p style={{ fontFamily: "'Syne', sans-serif", fontSize: isMobile ? 26 : 30, fontWeight: 700, color: s.textColor, lineHeight: 1, marginBottom: 6 }}>{value}</p>
                <p style={{ fontSize: isMobile ? 10 : 11, color: s.subColor }}>
              ...
              {recentUpdates.slice(0, 6).map((update) => (
                <div key={update.id} style={{ display: 'flex', gap: 12, padding: '14px 18px', borderBottom: '1px solid #f9fafb', borderRight: '1px solid #f9fafb' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {update.notes && (
                      <p style={{ fontSize: 11.5, color: '#9ca3af', marginTop: 3, fontStyle: 'italic', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', margin: 0 }}>"{update.notes}"</p>
                    )}
...
function DonutChart({ data, total, colors }: { data: Record<string, number>; total: number; colors: Record<string, string> }) {
  ...
  return (
    <svg width="120" height="120" viewBox="0 0 120 120">
      {total === 0
        ? <circle cx={cx} cy={cy} r={r} fill="#f0f4f0" />
